...
final class QuotaViewModel: ObservableObject {
    ...
    @Published private(set) var dialStyle: QuotaDialStyle

    private let store = QuotaStore()
    private let dialStyleKey = "quotaDialStyle"
    private lazy var reader = CodexRateLimitReader(store: store)
    private var refreshTask: Task<Void, Never>?
    private var timerTask: Task<Void, Never>?

    init() {
        let savedStyle = UserDefaults.standard.string(forKey: dialStyleKey)
            .flatMap(QuotaDialStyle.init(rawValue:))
        dialStyle = savedStyle ?? .casio
    ...
    func setDialStyle(_ style: QuotaDialStyle) {
        dialStyle = style
        UserDefaults.standard.set(style.rawValue, forKey: dialStyleKey)
    }
